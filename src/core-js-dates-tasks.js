...
function getNextFriday(date) {
  ...
  let toFriday = 5 - weekday;
  if (toFriday < 1) {
    toFriday = 12 - weekday;
  }
  const sevenDays = toFriday * 24 * 60 * 60 * 1000;
  return new Date(+date + sevenDays);
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const differenceDateMs = +new Date(dateEnd) - +new Date(dateStart);
  return 1 + differenceDateMs / day;
}
...
function getWeekNumberByDate(date) {
  ...
  const differenceInDays = Math.round(
    (date - startYear) / (1000 * 60 * 60 * 24)
  );
  const weekNumber = Math.floor(differenceInDays / 7) + 1;
  return weekNumber + weekPlus;
}
...
function getNextFridayThe13th(date) {
  ...
  while (currentDate.getDay() !== 6 || currentDate.getDate() !== 14) {
    currentDate.setDate(currentDate.getDate() + 1);
  }
  const result = new Date(+currentDate - 24 * 60 * 60 * 1000);
  return result;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const endDate = new Date(
    +new Date(endYear, endMonth - 1, endDay) + 60 * 60 * 1000
  );
  const resultDates = [];
  ...
  while (startDate <= endDate) {
    let days = 0;
    let isLast = 1;
    while (days < countWorkDays && isLast < endDate) {
      const workDay = new Date(+startDate + days * 24 * 60 * 60 * 1000);
      resultDates.push(workDay);
      days += 1;
      isLast = workDay;
    }
    startDate = new Date(
      +startDate + (countOffDays + days) * 24 * 60 * 60 * 1000
    );
  }
  return resultDates.map((val) => {
